...
export default function ProveedoresPage() {
  ...
  const filtered = proveedores.filter((p) => {
    ...
    const matchSearch = p.nombre.toLowerCase().includes(q) || (p.rfc ?? '').toLowerCase().includes(q)
    ...
    <div className="max-w-2xl mx-auto px-4 py-5">
      ...
      <div className="flex flex-col gap-2 mb-4">
        ...
        <div className="flex gap-1.5">
          ...
          <span className="ml-auto text-xs text-[var(--nm-text-subtle)] self-center">{filtered.length} proveedor{filtered.length !== 1 ? 'es' : ''}</span>
        </div>
      </div>
            ...
            <div key={p.id} className={`nm-card p-4 ${p.activo ? 'border-gray-200' : 'border-gray-100 opacity-60'}`}>
              <div className="flex justify-between items-start gap-2">
                ...
                <div className="flex flex-col gap-1 shrink-0">
                  ...
                  <button onClick={() => openEdit(p)} className="px-3 py-1.5 text-xs font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg">Editar</button>
